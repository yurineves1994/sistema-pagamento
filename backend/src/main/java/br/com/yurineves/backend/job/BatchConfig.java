package br.com.yurineves.backend.job;
...
import br.com.yurineves.backend.entities.TipoTransacao;
import br.com.yurineves.backend.entities.Transacao;
import br.com.yurineves.backend.entities.TransacaoCNAB;
...
public class BatchConfig {
  ...
  // TRANSACAO CNAB É O FORMATO DE ENTRADA DO ARQUIVO CNAB
  // TRANSACAO É O FORMATO DE SAIDA DO ARQUIVO CNAB
  @Bean
  Step step(
  ...
  JdbcBatchItemWriter<Transacao> writer(DataSource dataSource) {
    return new JdbcBatchItemWriterBuilder<Transacao>()
        ...
        .sql("""
              INSERT INTO transacao(tipo, data, valor, cpf, cartao,
               hora, dono_loja, nome_loja
               ) VALUES (:tipo, :data, :valor, :cpf, :cartao, :hora, :donoDaLoja, :nomeDaLoja)
            """)
        .beanMapped() // é usado esse beanMapped porque os nomes dos placeholders (:tipo, :data, etc)
                      // são iguais aos atributos da classe
        .build();
  }
